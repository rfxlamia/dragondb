...
- [x] **DetailContent modals host.** Swift: `DetailContentModalsWrapper` on `MainSplitView`. Tauri: **DONE** `QueryResultsPane` now receives `raw`/`dateFormat`/`query`/`sourceTable`/`columnMetadata`/pagination/`onUpdateRow`/`onDeleteRows`/`onSaveCsv` from `AppWorkspace`/`App.tsx`, so the JSON viewer, row editor, and delete-confirm modals attach to the results shell and call `ipc.updateRow`/`ipc.deleteRows`/`ipc.saveCsvFile`. **Done:** JSON / row editor / delete confirm attach to the shell that owns the grid. **SEQUENCE**.
...
- [x] **Edit row sheet.** Swift: `RowEditorView` PK read-only + badge, NULL checkbox if nullable, date/time pickers by type, Save disabled while saving, error alert. Tauri: **DONE** `RowEditor` receives full `ColumnInfo[]`; PK fields are read-only with a Primary key badge; nullable columns expose a labelled NULL toggle that restores the last non-null draft; timezone-free `date` / `time without time zone` / `timestamp without time zone` use native controls; timezone-bearing and unknown types stay exact text. Save is single-flight (pending disables fields/Save/Cancel; duplicate click/Enter does not start a second mutation). Rejected `updateRow` keeps the dialog and draft and shows an accessible human error. Committed update/delete with a failed page reload closes the editor and offers reload-only retry (`run-row-operation-on-active-tab`, never repeats the mutation). **SEQUENCE**.
...
| Result-grid editability | `QueryEditability` + `RowOperationsService` | Util **DONE**; IPC `updateRow`/`deleteRows` **DONE**; UI **DONE** — Edit/Delete attach via `columnMetadata` primary keys and `determineEditability`; the six `RowOperationError` kinds map to accessible human copy (`row-operation-error.ts`); rejected Save keeps the draft; confirmed mutation + failed reload is retry-load only. Context-mismatch help **MISSING** (approved deviation: browse results/page/generation clear on successful database/connection change; ordinary SQL/canvas results are preserved) | Edit/delete only when `isEditable`; six `RowOperationError` kinds surface | SEQUENCE | SHIP — **done** (context-mismatch help remains approved deviation) |
