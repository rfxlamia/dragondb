...
- [x] **Background persist.** Swift: `scenePhase == .background` → `saveCurrentStateToTab` + `cleanupOnWindowClose`. Tauri: **MISSING** UI hook; `tabs.persistTab` exists. **Done:** hiding/closing the window persists active tab query text (and does not persist results unless `includeCachedResults`). **SEQUENCE**.
...
- [x] **Mutation toast overlay.** Swift: `MainSplitView` bottom-trailing `MutationToastView` when `appState.query.mutationToast != nil`; View Table selects table + `requestTableQuery` if already selected; auto-dismiss 5s (`QueryState.showMutationToast`). Tauri: **MISSING**. **Done:** successful mutation with 0 returned rows shows toast with title from `QueryType.successTitle`; DROP TABLE hides View Table (`MutationToastData.showViewTableButton`). **SEQUENCE**.
...
**Still SEQUENCE after this slice:** collapsible connection sidebar; SQL hatch; Show All Rows / table expander; grid mutation/filter/sort/JSON/dates; saved-query filter/sort/duplicate; tab title labels; persist visual IR; database picker; loading overlay; history relative-date labels (epoch `created_at`).
