...
- **MISSING UI:** remaining SEQUENCE (table browse Show All Rows / SQL hatch / **collapsible connection sidebar** / database picker / loading overlay / client filter / column sort / multi-row select). Workspace-chrome spec: `docs/pocket/spec/2026-08-15-sp4b-workspace-chrome/workspace-chrome.md`.
...
- [x] **Editability + context-mismatch disable.** Swift: `determineQueryEditability`; toolbar popover `DetailContentViewModel.contextMismatchHelpText`; `QueryEditabilityTests`. Tauri: **DONE** — `QueryResultsPane` now calls `determineEditability(query, { sourceTable })` with real `query`/`sourceTable` (previously always `undefined`/`""`, so Edit/Delete were always in the join/CTE-disabled UI state); the reason title surfaces as the button `title` tooltip, not a Swift-style popover. **Done:** JOIN/CTE/aggregate/etc. disable edit/delete with Swift reason titles; results from another connection/db show the mismatch help. **SEQUENCE**.
